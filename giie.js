    ...
    try {
        ...
        if (!mek.key.remoteJid.endsWith('@g.us') && offline) {
            if (!mek.key.fromMe) {
                ...
                hooh = await fetchJson(`https://api.zeks.me/api/dropwater?apikey=${ZeksApi}&text=${pushname}`)
                ganz = await getBuffer(hooh.result)
                menu = `*_Halo ${pushname}_* 🤗\n_Maaf saat ini saya (echa) sedang offline_\n_Pesan kamu akan di balas ketika online_\nTerimakasih 🙏`
                buttons = [{
                    buttonId: `.giie`,
                    buttonText: {
